...
const faqs = [
  ...
  {
    q: '发票/车票能拍照自动识别填表吗？',
    a: '能 —— 两个入口：① 录入/编辑弹窗内点「📷 传图识别预填」选票据照片，自动把日期、车次、座位等级、金额、出发/到达地等票面信息填进对应字段；② 已上传过图片附件的行卡片上，图片旁有「🔍识别」，点一下即识别并自动打开编辑弹窗预填。识别只做「预填」：人工核对后点保存才落库；看不清的字段模型宁可不填，请以票面为准。安全边界：出钱人（谁垫付）、统一支付涵盖范围、购买人、备注 永远不会被识别结果改动。仅支持 jpg/png/webp/gif/bmp 图片，PDF 请截图或拍照转成图片再识别；免费模型偶发限流（提示稍后重试），服务端未配置识别时按钮会提示联系负责人。',
  },
];
</script>
    ...
    <div class="exp-wrap guide">
      ...
      <section class="card g-sec">
        ...
        <ul class="g-ul">
          ...
          <li>传图识别：录入/编辑弹窗可「📷 传图识别预填」（拍发票/车票照片自动填字段）；已传图片附件旁可「🔍识别」打开编辑核对预填 —— 均先人工确认、保存才落库；出钱人/涵盖范围/购买人/备注永不被自动改，看不清宁可不填（见 FAQ）。</li>
