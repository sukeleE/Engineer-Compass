<script setup>
// 报销整理模块 · 详细使用教程（2026-09-06 版：成员权限收紧为只可自理自己名下）
...
const faqs = [
  ...
  {
    q: '队员可以帮别人录吗？' ,
    a: '不能直接帮 —— 2026-09-06 起队员新增/改/删一律只限「自己名下」：添加记录时出钱人固定=你本人（没有下拉），本队队友已填的、跨队的、全项目区他人的行都只读，改不了也删不了（他人已填的费用信息只有负责人能删改）。队友请假了/公用开销/代他人垫付，请找负责人代录 —— 负责人可给任何成员代录与纠错；负责人代录后挂在你名下的行，你自己可以再编辑。',
  },
  {
    q: '录错了/重复录了怎么删？',
    a: '行卡片右下角有 🗑 删除按钮（谁有权限见上方身份表）：负责人可删任意行；队员只能删自己名下的行（他人的行没有删除按钮，服务端同样拦截）；删除会连同该行已传附件一起清掉，删前有二次确认。误删可让负责人补录一条，删除本身不可恢复。',
  },
  {
    q: '认领后换浏览器/换手机，身份还在吗？',
  ...
  {
    q: '车次忘填了要紧吗？',
    a: '不影响金额、统计与 Excel/附件 —— 页面顶部「同车次车票」会把没填车次的车票归到「未填车次」一组统一汇总，一眼能看出哪些还没补。车票建议都填上车次（票面上就有，如 G1234），填好后自动按「出发日期＋车次＋座位等级」归组方便核对；历史行随时点卡片「✏ 编辑」补填即可。',
  },
  {
    q: '发票/车票能拍照自动识别填表吗？',
    a: '能 —— 两个入口：① 录入/编辑弹窗内点「📷 传图/PDF 识别预填」选票据照片或发票 PDF，自动把日期、车次、座位等级、金额、出发/到达地等票面信息填进对应字段；② 已上传过图片/PDF 附件的行卡片上，文件旁有「🔍识别」，点一下即识别并自动打开编辑弹窗预填。识别只做「预填」：人工核对后点保存才落库；看不清的字段模型宁可不填，请以票面为准。安全边界：出钱人（谁垫付）、统一支付涵盖范围、购买人、备注 永远不会被识别结果改动。图片（jpg/png/webp/gif/bmp）与 PDF 均可识别，电子发票 PDF 直接传、不用截图；免费视觉模型偶发限流（提示稍后重试），服务端未配置识别时按钮会提示联系负责人。',
  },
  {
    q: 'PDF 电子发票能识别吗？要不要先截图转成图片？',
    a: '不用转图，PDF 直接传就能识别。数字版电子发票（税务平台/航司/铁路导出）自带文字层，走文字识别，又快又准；若 PDF 是「拍照或截图后转成的」（扫描件，没有文字层），系统会自动转成图片走视觉识别 —— 你不需要分辨自己手里是哪种，两种都能填表。两点注意：多页 PDF 只识别前 3 页（发票以单页为主，超出会弹提示）；加密/加了打开密码的 PDF 请先解除密码再传。',
  },
];
</script>
  ...
  <div class="expense-page">
    <div class="exp-wrap guide">
      ...
      <section class="card g-sec">
        ...
        <p class="callout info">💡 一句话分工：<b>负责人管结构</b>（建队/名单/截止/导出/代录纠错），<b>队员管自己的账</b>（认领后只能新增/改/删自己名下的行，本队与全项目区同口径）；不认领只能看，不能写。</p>
      </section>
      ...
      <section class="card g-sec">
        ...
        <div class="role-cards">
          ...
          <div class="role-card member">
            ...
            <ul>
              <li>新增 / 改 / 删 / 传附件：<b>只能对自己名下的行</b>（出钱人自动=本人，本队与全项目区同口径）</li>
              <li>他人已填的行 —— 本队队友 / 公用行 / 跨队 / 项目级 —— 一律只读（仅负责人可删改）</li>
              <li>替人垫付或公用开销：找负责人代录到你名下即可自理</li>
      ...
      <section class="card g-sec">
        ...
        <div class="cat-grid">
          ...
          <div class="cat-card"><h3>② 车票</h3><p>出发/到达<b>日期</b>（日历选，跨天自动隔开）、出发地/到达地、<b>车次</b>（如 G1234，票面上有）、座位等级、金额。往返票分两行录；填了车次，页面顶部会自动按「出发日期＋车次＋座位等级」把同趟车票聚合起来，方便按趟核对张数与金额。</p><p class="att">附件：发票(PDF) / 付款凭证(截图)</p></div>
      ...
      <section class="card g-sec">
        ...
        <div class="tips-grid">
          ...
          <p class="callout warn">⚠ 队员们注意：你录的每一笔账都<b>只能挂自己名下</b>（出钱人固定=你本人，没有下拉框）—— 不管在哪个队伍标签还是全项目统一支付区。队友已填的、跨队的、公用「队伍」行一律只读，改/删/传附件只有负责人能做；代他人垫付或公用开销请找负责人代录，挂在你名下的行（含负责人代录的）你自己都能编辑纠错。</p>
        </div>
      </section>
      ...
      <section class="card g-sec">
        ...
        <ul class="g-ul">
          ...
          <li>识别预填：录入/编辑弹窗可「📷 传图/PDF 识别预填」（拍发票/车票照片，或直接选<b>电子发票 PDF</b>，自动填字段）；已传图片/PDF 附件旁可「🔍识别」打开编辑核对预填 —— 均先人工确认、保存才落库；出钱人/涵盖范围/购买人/备注永不被自动改，看不清宁可不填（见 FAQ）。</li>
      ...
      <section class="card g-sec">
        ...
        <p class="g-p">若填了<b>车次</b>，统计条下方还会按「出发日期＋车次＋座位等级」把<b>全项目</b>车票聚合展示（跨队伍、含全项目统一支付区；没填车次的归「未填车次」一组）—— 金额口径与统计条 ②小计一致，仅页面参考、不进 Excel。</p>
        <table class="g-tb">
      ...
      <p class="g-foot">教程覆盖版本：2026-09-04（队伍横向标签 + 全项目统一支付自理）· 2026-09-06（收紧：队员只可自理自己名下，他人已填的信息仅负责人可删改）· 2026-09-10（车次字段＋同车次聚合＋票据图片识别预填）。页面有不懂的按钮 → 鼠标悬停通常有提示；还有疑问找负责人。</p>
    </div>
  </div>
</template>
